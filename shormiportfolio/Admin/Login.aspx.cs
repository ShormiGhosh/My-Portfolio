...
namespace shormiportfolio.Admin
{
    public partial class Login : System.Web.UI.Page
    {
        ...
        protected void btnLogin_Click(object sender, EventArgs e)
        {
            ...
            if (!isValidUser)
            {
                // Check against web.config values as fallback
                ...
                if (username.Equals(configUsername, StringComparison.OrdinalIgnoreCase) && 
                    password.Equals(configPassword, StringComparison.Ordinal))
                {
                    isValidUser = true;
                    ShowError("?? Logged in using fallback authentication. Database login failed, but web.config values worked. Check your database setup.");
                }
            }
            ...
            {
                ShowError("? Invalid credentials. Please check: 1) Database is created 2) Admin user exists 3) Credentials are correct (admin/admin123)");
        ...
        private void CreateUserPreferenceCookies()
        {
            ...
            string loginCountStr = GetCookieValue("LoginCount", "0");
            int loginCount;
            if (!int.TryParse(loginCountStr, out loginCount))
            {
                loginCount = 0;
            }
            CreateCookie("LoginCount", (loginCount + 1).ToString(), 30);
        }
        ...
        private bool ValidateUser(string username, string password)
        {
            try
            {
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    // First, check if AdminUsers table exists and has data
                    ...
                    using (SqlCommand checkCmd = new SqlCommand(checkTableQuery, conn))
                    {
                        ...
                        if (tableExists == 0)
                        {
                            ShowError("AdminUsers table not found. Please run the database creation script first.");
                            return false;
                        }
                    }
                    
                    // Check how many admin users exist in total
                    ...
                    using (SqlCommand countCmd = new SqlCommand(countQuery, conn))
                    {
                        ...
                        if (totalUsers == 0)
                        {
                            ShowError("No admin users found. Please run the database setup script.");
                            return false;
                        }
                    }
                    
                    // Enhanced validation with detailed debugging
                    string query = @"SELECT AdminID, Username, Password, Email 
                                    FROM AdminUsers 
                    ...
                    using (SqlCommand cmd = new SqlCommand(query, conn))
                    {
                        ...
                        using (SqlDataReader reader = cmd.ExecuteReader())
                        {
                            ...
                            {
                                // User not found - check if username exists but password is wrong
                                reader.Close();
                                string usernameCheckQuery = "SELECT COUNT(*) FROM AdminUsers WHERE LTRIM(RTRIM(Username)) = @Username";
                                using (SqlCommand usernameCmd = new SqlCommand(usernameCheckQuery, conn))
                                {
                                    usernameCmd.Parameters.AddWithValue("@Username", username.Trim());
                                    int usernameCount = (int)usernameCmd.ExecuteScalar();
                                    
                                    if (usernameCount > 0)
                                    {
                                        ShowError("Invalid password for user: " + username);
                                    }
                                    else
                                    {
                                        ShowError("Username not found: " + username);
                                    }
                                }
                                return false;
                            }
            ...
            {
                // SQL-specific errors
                ShowError($"Database error: {sqlEx.Message}. Please check your database connection.");
